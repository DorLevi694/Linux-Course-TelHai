...
int makeMeServerAndListen(struct sockaddr_in *myAddrass,in_port_t port){
  ...
  myAddrass->sin_addr.s_addr = INADDR_ANY;
  myAddrass->sin_family = AF_INET;
  myAddrass->sin_port = htons( port );

  if (bind(socket_fd , (struct sockaddr *)myAddrass , sizeof(*myAddrass))<0){
